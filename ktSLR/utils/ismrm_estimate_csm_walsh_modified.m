function [csm] = ismrm_estimate_csm_walsh(img, smoothing,threshold)
%
...
magsmooth=conv2(mag,h_smooth,'same');
mask = magsmooth > threshold*max(magsmooth(:));
mask = bwmorph(mask,'dilate',4);
s_raw=img ./ repmat(mag + eps,[1 1 ncoils]); 
s_raw= s_raw.*repmat(mask,[1 1 ncoils]);
clear mag; 
